...
set(OPTIONS "${OPTIONS} --enable-avutil")
set(FFMPEG_PKGCONFIG_MODULES libavutil)

set(OPTIONS "${OPTIONS} --enable-static")
...
if("amf" IN_LIST FEATURES)
    ...
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_amf,hevc_amf,av1_amf")
    set(WITH_AMF OFF)
else()
    set(OPTIONS "${OPTIONS} --disable-amf")
    set(OPTIONS "${OPTIONS} --disable-encoder=h264_amf,hevc_amf,av1_amf")
    set(WITH_AMF ON)
endif()
...
if("nvcodec" IN_LIST FEATURES)
    ...
    if(NOT (VCPKG_TARGET_IS_WINDOWS))
        set(OPTIONS "${OPTIONS} --enable-cuda_llvm")
    endif()
    set(WITH_NVCODEC ON)
else()
    set(OPTIONS "${OPTIONS} --disable-cuda --disable-nvenc --disable-nvdec --disable-cuvid --disable-ffnvcodec")
    set(OPTIONS "${OPTIONS} --disable-encoder=h264_nvenc,hevc_nvenc,av1_nvenc")
    if(NOT (VCPKG_TARGET_IS_WINDOWS))
        set(OPTIONS "${OPTIONS} --disable-cuda_llvm")
    endif()
...
if(VCPKG_TARGET_IS_WINDOWS)
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_mf,hevc_mf")
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_qsv,hevc_qsv,av1_qsv")
elseif(VCPKG_TARGET_IS_IOS OR VCPKG_TARGET_IS_OSX)
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_videotoolbox,hevc_videotoolbox")
elseif(VCPKG_TARGET_IS_LINUX)
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_vaapi,hevc_vaapi,av1_vaapi")
    set(OPTIONS "${OPTIONS} --enable-encoder=h264_v4l2m2m")
endif()
